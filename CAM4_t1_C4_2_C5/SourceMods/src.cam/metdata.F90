  ...
  !----------------------------------------------------------------------- 
    ...
    integer :: nudging_ss_counter = 00 !ms! for snapshot nudging at exactly 6h 
!    integer :: nudging_ss_counter = 100 !ms! for snapshot nudging at 6h+dt
  ...
  !
    ...
    if ( curr_mod_time > datatimep ) then
      ...
      do while ( ll_exist == .false. )
        call sleep(1)
        isleep = isleep + 1
        inquire(file = "is_cam_nudging_data_ready", exist = ll_exist)
...
!MS!          call open_met_datafile( grid, curr_filename, curr_fileid,curr_data_times, met_datapath )
...
!! ------------------------------------tbd
      ...
      if (masterproc) then
        open (unit = i, file = "is_cam_nudging_data_ready")
